  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ height: "180px", background: "#f5f0eb" }}
      >
        <Image
          src={item.image}
          alt={item.name}
      ...
      <div className="flex flex-col flex-1 p-3.5 sm:p-4">
        ...
        <h3
          className="text-lg sm:text-xl leading-snug mb-1.5"
          style={{ fontFamily: "var(--font-cormorant), serif", fontWeight: 400, color: "#1c1410" }}
        >
          {item.name}
        </h3>
        <p className="text-[0.72rem] leading-relaxed line-clamp-2 mb-auto" style={{ color: "#7a6a5e" }}>
          {item.description}
        </p>
          ...
          <div className="flex gap-2 mt-3">
            {(["30", "40"] as const).map((s) => (
              <button
                key={s}
                onClick={() => setSize(s)}
                className="flex-1 py-2 rounded-sm text-[0.65rem] tracking-wider uppercase transition-all duration-200"
                style={{
        ...
        >
          ...
          <button
            onClick={handleAdd}
            className="dish-add-btn flex items-center gap-1.5 px-3 py-2 rounded-sm text-[0.6rem] tracking-wider uppercase transition-all duration-300 flex-shrink-0"
            style={{
          ...
          >
            ...
            <span className="hidden xs:inline">{added ? "Додано" : "В кошик"}</span>
          </button>
        </div>
      </div>
    </div>
...
export default function MenuPage() {
  ...
  return (
    <div className="pt-16 sm:pt-20" style={{ background: "#faf7f2" }}>
      ...
      {/* Заголовок */}
      <div className="max-w-7xl mx-auto px-5 sm:px-6 pt-8 sm:pt-12 pb-6 sm:pb-8">
      ...
      <div
        ref={navRef}
        className="sticky z-30 top-16 sm:top-20"
              ...
              return (
                <div key={cat.id} className="flex items-center">
                  <button
                    ...
                    className="menu-nav-item flex-shrink-0 flex flex-col items-center px-4 sm:px-6 py-3.5 sm:py-4 relative transition-all duration-300"
                    style={{ color: isActive ? "#1c1410" : "#a09080" }}
                  >
                    <span className="text-[0.68rem] sm:text-[0.72rem] tracking-[0.1em] sm:tracking-[0.12em] uppercase font-medium whitespace-nowrap">
                      {cat.label}
                    </span>
                    <span
        ...
        {grouped.length === 0 ? (
          ...
          grouped.map((cat, i) => (
            <section
              ...
              className="pt-12 sm:pt-14 pl-4 sm:pl-5 relative"
              style={{ borderLeft: "2px solid #e8ddd4" }}
            >
              <span
                className="absolute left-[-5px] top-[3.25rem] sm:top-[3.75rem] w-2 h-2 rounded-full flex-shrink-0"
              ...
              {/* Сітка: 2 колонки на мобілі, 3 на планшеті, 4 на великому */}
              <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
                {cat.items.map((item) => <DishCard key={item.id} item={item} />)}
              </div>

              {i < grouped.length - 1 && (
                <div className="mt-12 sm:mt-14 mb-2 flex flex-col items-center gap-2">
                  ...
                  <span className="text-[0.48rem] tracking-[0.35em] uppercase" style={{ color: "#d8ccc0" }}>Brasa</span>
                </div>
              )}
            </section>
          ))
